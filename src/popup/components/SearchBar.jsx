  ...
  return (
    <div className="relative">
      <div className={`relative flex items-center group ${isFullPage ? '' : 'max-w-md mx-auto'}`}>
        <div className={`absolute left-4 transition-colors ${isSearching ? 'text-[#C49A6C]' : 'text-[#A8A29E]'}`}>
        ...
        <div className="absolute right-4 flex items-center gap-2">
          {query && (
            <button 
              onClick={() => setQuery('')}
              className="p-1 text-[#A8A29E] hover:text-[#3D3832] transition-colors"
            >
              <X size={18} />
            </button>
          )}
          <AnimatePresence>
            {isAISearch && (
              <motion.div
                ...
                className="bg-[#E8E2D6] text-[#C49A6C] px-2 py-1 rounded-lg text-[10px] font-black uppercase tracking-tighter flex items-center gap-1 border border-[#C49A6C]/20 shadow-sm"
              >
      ...
      <AnimatePresence>
        {results && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 10 }}
            className={`absolute top-full left-0 right-0 mt-2 bg-white border border-[#E8E2D6] rounded-2xl shadow-2xl z-50 overflow-hidden max-h-[400px] overflow-y-auto ${isFullPage ? '' : 'max-w-md mx-auto'}`}
          >
            {results.length === 0 ? (
              ...
              <div className="py-2">
                {results.map((res) => (
                  <button
                    ...
                    className="w-full text-left p-4 hover:bg-[#FAF8F5] transition-colors flex items-start gap-3 border-b border-[#E8E2D6] last:border-0"
                  >
                    <div className="mt-1 text-[#C49A6C]">
                      <Sparkles size={14} className={res.score > 0.8 ? 'fill-current' : ''} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h5 className="text-sm font-bold text-[#3D3832] truncate">{res.title}</h5>
                      {res.summary && <p className="text-xs text-[#A8A29E] line-clamp-1 mt-0.5">{res.summary}</p>}
                    </div>
                  </button>
                ))}
              </div>
